                ...
                <div className="marquee-track flex h-full items-center gap-4 sm:gap-5">
                  {marqueeItems.map((client, index) => (
                    <motion.div
                      ...
                      className="min-w-[220px] max-w-[220px] h-[220px] sm:min-w-[240px] sm:max-w-[240px] sm:h-[240px] rounded-2xl border border-border/70 bg-card/70 p-5 text-center shadow-sm shadow-primary/5 grayscale transition-all duration-300 hover:grayscale-0 flex flex-col justify-between"
                      aria-hidden={index >= clients.items.length}
                    >
                      <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl border border-border/70 bg-white/70 text-lg font-semibold tracking-[0.3em] text-muted-foreground transition-all duration-300 group-hover:text-primary">
                        {client.name.slice(0, 2).toUpperCase()}
                      </div>
                      <div>
                        <h3 className="mt-4 font-heading text-base font-semibold">{client.name}</h3>
